...
async function buildNextFixture(dir: string): Promise<void> {
  ...
  await writeFile(join(dir, "src", "app", "layout.tsx"), "export default function RootLayout({ children }: { children: React.ReactNode }) { return children; }");
  await writeFile(join(dir, "src", "app", "page.tsx"), "export default function Home() { return null; }");
  ...
  await writeFile(join(dir, "src", "app", "(dashboard)", "settings", "exceptions.tsx"), "export default function ExceptionsPage() { return null; }");
...
describe("next-fixture: pack lookalike_ignore suppresses framework noise", () => {
  ...
  it("WITHOUT pack default ignore (--ignore '' override not possible, but validates via bare lookalike test)", async () => {
    ...
    await writeFile(join(dir, "src", "app", "settings", "exceptions.tsx"), "export default function P() { return null; }");
