...
describe("CLAUDE.md hybrid+markdown sync (fragment marker bug)", () => {
	...
	it("sync after adopt: CLAUDE.md shows skip (in sync), never abort", async () => {
		...
		// Must report the pack-managed section as in sync (#34: CLAUDE.md now lands at
		// .claude/CLAUDE.md by default, so the log line shows the canonical → resolved
		// path mapping). #592: reason is consumer-legible, not internal glossary.
		expect(r.stdout).toMatch(/skip: CLAUDE\.md.* — pack-managed section unchanged/);
	});
});
...
describe("settings.json hybrid+json preservation", () => {
	...
	it("sync after modifying permissions: permissions still preserved, hooks still pack-owned", async () => {
		...
		const sync = await runCli(["sync", "--offline-fixture", "packs/next-react"], {
			cwd: dir,
			stdin: "y\n",
		});
	...
	it("CrewOps regression: sync preserves PreToolUse validators and SessionStart banner", async () => {
		...
		const postCommands = settings.hooks.PostToolUse[0].hooks.map(
...
describe("#192 — sync runs without confirmation prompt", () => {
	...
	it("sync --dry-run renders preview without applying changes", async () => {
		...
		const r = await runCli(["sync", "--offline-fixture", "packs/next-react", "--dry-run"], {
			cwd: dir,
		});
		expect(r.code).toBe(0);
...
describe("Issue #138 — sync reports packVersion from .claude-ds.json", () => {
	...
	it("#138 sync complete message shows packVersion from config", async () => {
		await writeFile(
			join(dir, ".claude-ds.json"),
			JSON.stringify({ packVersion: "v2.0.0", pack: "next-react", mode: "warn" }),
		);
		const r = await runCli(["sync", "--offline-fixture", "packs/next-react"], {
			cwd: dir,
			stdin: "y\n",
	...
	it("#138 legacy 'version' key is reported correctly in completion message", async () => {
		await writeFile(
			join(dir, ".claude-ds.json"),
			JSON.stringify({ version: "v1.5.0", pack: "next-react", mode: "warn" }),
		);
		const r = await runCli(["sync", "--offline-fixture", "packs/next-react"], {
			cwd: dir,
			stdin: "y\n",
