...
describe("audit --fix", () => {
	...
	it("reports unfixable findings as requiring manual intervention", async () => {
		...
		await writeFile(
			answersPath,
			JSON.stringify({
				"DRIFT-DS-IMPORTS-FEATURE:design-system/composites/solo-label.tsx::convert:@/features/billing/api:api":
					"defer",
			}),
		);
		const r = await runCli(["audit", "--pack", "next-react", "--fix", "--answers", answersPath], {
			cwd: dir,
		});
	...
	it("exits 1 when unfixable findings remain after --fix", async () => {
		...
		await writeFile(
			answersPath,
			JSON.stringify({
				"DRIFT-DS-IMPORTS-FEATURE:design-system/composites/orphan.tsx::convert:@/features/billing/api:api":
					"defer",
			}),
		);
...
describe("audit --except", () => {
	...
	it("creates bare exceptions when --except is used without --issue or --permanent", async () => {
		...
		const r = await runCli(
			["audit", "--pack", "next-react", "--except", "--reason", "just a reason"],
			{ cwd: dir },
		);
		expect(r.code).toBe(0);
		const raw = await readFile(join(dir, "design-system/exceptions.json"), "utf8");
	...
	it("appends to existing exceptions.json without overwriting", async () => {
				...
				exceptions: [
					{
						...
						path: "design-system/composites/another.tsx",
						issue: "#10",
						reason: "existing",
					},
				],
		...
		const r = await runCli(
			["audit", "--pack", "next-react", "--except", "--reason", "new workaround", "--issue", "#99"],
			{ cwd: dir },
		);
		expect(r.code).toBe(0);
		const raw = await readFile(join(dir, "design-system/exceptions.json"), "utf8");
		...
		expect(
			parsed.exceptions.find(
				(e: { path: string }) => e.path === "design-system/composites/solo-label.tsx",
			),
		).toBeTruthy();
		expect(
			parsed.exceptions.find(
				(e: { path: string }) => e.path === "design-system/composites/another.tsx",
			),
		).toBeTruthy();
	});
	...
	it("is a no-op when --except is passed but all findings are already suppressed", async () => {
		...
		await writeFile(
			...
			JSON.stringify({
				exceptions: [
					{
						rule: "DRIFT-MISPLACED",
						path: "design-system/composites/solo.tsx",
						issue: "#1",
						reason: "existing",
					},
				],
		...
		const r = await runCli(
			["audit", "--pack", "next-react", "--except", "--reason", "unused", "--issue", "#2"],
			{ cwd: dir },
		);
		expect(r.code).toBe(0);
	...
	it("exits 0 after writing exceptions (all findings suppressed)", async () => {
		...
		const r = await runCli(
			["audit", "--pack", "next-react", "--except", "--reason", "wontfix", "--issue", "#1"],
			{ cwd: dir },
		);
		expect(r.code).toBe(0);
	});
...
describe("audit --fix --except", () => {
	...
	it("fixes fixable items first, then writes exceptions for the rest", async () => {
		...
		const r = await runCli(
			["audit", "--fix", "--except", "--reason", "pending move", "--issue", "#55"],
			{ cwd: dir },
		);
		expect(r.code).toBe(0);
		...
		const parsed = JSON.parse(raw);
		expect(
			parsed.exceptions.some((e: { rule: string }) => e.rule === "DRIFT-PATTERN-NO-SLOTS"),
		).toBe(true);
	});
});
...
describe("audit --fix stale exception cleanup", () => {
	...
	it("preserves exceptions that still fire after fixes", async () => {
		...
		await writeFile(
			...
			JSON.stringify({
				exceptions: [
					{
						rule: "DRIFT-MISPLACED",
						path: "design-system/composites/orphan.tsx",
						issue: "#20",
						reason: "known",
					},
				],
		...
		const raw = await readFile(join(dir, "design-system/exceptions.json"), "utf8");
		const parsed = JSON.parse(raw);
		expect(parsed.exceptions.some((e: { rule: string }) => e.rule === "DRIFT-MISPLACED")).toBe(
			true,
		);
	});
});
