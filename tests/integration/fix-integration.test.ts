...
async function collectFindings(cwd: string): Promise<DriftFinding[]> {
	...
	for (const tierDir of tierDirs) {
		let entries: string[];
		try {
			entries = await readdir(join(cwd, tierDir));
		} catch {
			continue;
		}
		for (const entry of entries) {
			...
			let source: string;
			try {
				source = await rf(join(cwd, filePath), "utf8");
			} catch {
				continue;
			}
...
describe("integration: full --fix pass on fixture project", () => {
	...
	it("resolves all drift categories in a single --fix pass", async () => {
		...
		const chipLines = Array.from({ length: 20 }, (_, i) => `  const v${i} = ${i};`).join("\n");
		await writeFile(
		...
		await writeFile(
			join(dir, "design-system/composites/card.tsx"),
			[
		...
		await writeFile(
			join(dir, "design-system/composites/badge.test.tsx"),
			[
				...
				exceptions: [
					{
						...
						path: "design-system/composites/badge.tsx",
						issue: "#99",
						reason: "tracked",
					},
				],
		...
		await writeFile(
			...
			['export * from "./button";', 'export * from "./input";', ""].join("\n"),
		);
		await writeFile(
		...
		const priceTagSource = await readFile(
			join(dir, "design-system/composites/price-tag.tsx"),
			"utf8",
		);
