...
describe("classify scope confinement (#209, PRD #241)", () => {
	...
	describe("--src brownfield pull-in — scope narrowing", () => {
		it("excludes lookalike_ignore paths from the walk", async () => {
			...
			await expect(access(join(dir, "design-system/atoms/private.tsx"))).rejects.toThrow();
			// Stayed where it was.
			await expect(
		...
		it("excludes the configured app dir from the walk", async () => {
			...
			// Sidebar stayed put.
			await expect(access(join(dir, "src/app/_components/sidebar.tsx"))).resolves.toBeUndefined();
			await expect(access(join(dir, "design-system/atoms/sidebar.tsx"))).rejects.toThrow();
		});
		...
		it("does NOT pull non-React .ts modules into design-system/atoms/", async () => {
			...
			await expect(access(join(dir, "design-system/atoms/format.ts"))).rejects.toThrow();
			await expect(access(join(dir, "design-system/atoms/util.test.ts"))).rejects.toThrow();

			// They stayed in src/components/.
			await expect(access(join(dir, "src/components/format.ts"))).resolves.toBeUndefined();
			await expect(access(join(dir, "src/components/util.test.ts"))).resolves.toBeUndefined();
		});
	});
});
