...
describe("retireStates migration op", () => {
  it("emits delete Changes for every *.states.json under design-system tiers", async () => {
    await scaffoldTiers();
    await writeFile(join(cwd, "design-system", "atoms", "Button.states.json"), `{ "__generated": "v1", "states": [] }`, "utf8");
    await writeFile(join(cwd, "design-system", "atoms", "Badge.states.json"), `{ "__generated": "v1", "states": [] }`, "utf8");
    await writeFile(join(cwd, "design-system", "composites", "Card.states.json"), `{ "__generated": "v1", "states": [] }`, "utf8");

    const changes = await retireStates.plan(makeCtx());
    const deletes = changes.filter((c): c is Extract<Change, { kind: "delete" }> => c.kind === "delete");
  ...
  it("idempotent: returns [] on a clean project (no states.json, no STATE-001, no meta.states)", async () => {
    await scaffoldTiers();
    await writeFile(join(cwd, "design-system", "exceptions.json"), JSON.stringify({ exceptions: [] }), "utf8");
  ...
  it("skips companion .tsx files (showcase, test)", async () => {
    ...
    await writeFile(join(cwd, "design-system", "atoms", "Button.showcase.tsx"), companionSource, "utf8");
