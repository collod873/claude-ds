...
let cwd: string;
beforeEach(async () => { cwd = await freshTmpDir("ds-folder-alias-"); });
...
function fakeCtx(srcRoot = "src"): ProjectContext {
  const cfg: Config = {
    ...
    domain_roots: ["features", "lib"], srcRoot,
  };
  return {
    cwd,
    cfg,
...
describe("dsFolderAlias op", () => {
  ...
  it("adds @ds/* to src/tsconfig.json when srcRoot=src and src/tsconfig.json exists", async () => {
    ...
    await writeFile(join(cwd, "src", "tsconfig.json"), JSON.stringify(tsconfig, null, 2) + "\n", "utf8");
