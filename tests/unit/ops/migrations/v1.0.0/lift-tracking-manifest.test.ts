...
describe("liftTrackingManifest.plan()", () => {
  ...
  it("lifts files[] from showcase manifest and writes tracking manifest", async () => {
    ...
    const polluted = {
      ...
      components: [{ name: "Button", tier: "atom", kind: "atom", path: "design-system/atoms/Button.tsx", path_no_ext: "design-system/atoms/Button", has_showcase: false, has_test: false }],
    ...
    expect(tracking.files[0]).toEqual({ path: "design-system/atoms/Button.tsx", category: "seeded" });
  ...
  it("merges into existing tracking manifest without duplicates", async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({ files: [{ path: "design-system/atoms/Button.tsx", category: "seeded" }] }, null, 2) + "\n",
    );
    ...
    expect(paths).toEqual(["design-system/atoms/Button.tsx", "design-system/atoms/Input.tsx"].sort());
  });
