...
import { proposeMetaRole, type ProposeMetaRoleOutcome } from "../../../src/lib/ops/propose-meta-role";
import { makeFakeCtx } from "../../helpers/fake-ctx";
import { cleanup, freshTmpDir } from "../../helpers/tmpdir";

let cwd: string;
beforeEach(async () => { cwd = await freshTmpDir("propose-meta-role-"); });
afterEach(async () => { await cleanup(cwd); });

function fakeCtx() {
  return makeFakeCtx(cwd, {
    exists: async (p: string) => { try { await stat(join(cwd, p)); return true; } catch { return false; } },
  });
}
