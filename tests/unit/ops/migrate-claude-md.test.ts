...
describe("migrateClaudeMd op — plan()", () => {
	it("first migration: root holds managed block, target absent → write target, delete root (claude-ds-owned shell)", async () => {
		...
		const writeTarget = changes.find((c) => c.kind === "write") as Extract<
			Change,
			{ kind: "write" }
		>;
	...
	it("root with extra user content: preserved on root, block moved to target", async () => {
		...
		const writeRoot = changes.find((c) => c.kind === "write" && c.path === "CLAUDE.md") as Extract<
			Change,
			{ kind: "write" }
		>;
		...
		const writeTarget = changes.find(
			(c) => c.kind === "write" && c.path === ".claude/CLAUDE.md",
		) as Extract<Change, { kind: "write" }>;
	...
	it("target already contains the marker: no target write, but root still cleaned", async () => {
		...
		expect(
			changes.find((c) => c.kind === "write" && c.path === ".claude/CLAUDE.md"),
		).toBeUndefined();
