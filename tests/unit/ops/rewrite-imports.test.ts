...
import { rewriteImports, fileImportsDsModule } from "../../../src/lib/ops/rewrite-imports";
...
let cwd: string;
beforeEach(async () => { cwd = await freshTmpDir("rewrite-imports-"); });
...
describe("rewriteImports op", () => {
  it("rewrites @/design-system/atoms/X → composites/X when X lives in composites", async () => {
    ...
    await writeFile(join(cwd, "design-system", "composites", "combobox.tsx"), `export const x = 1;`);
  ...
  it("idempotent: re-plan after apply returns []", async () => {
    ...
    await writeFile(join(cwd, "design-system", "atoms", "other.tsx"), `import "@/design-system/atoms/card";\n`);
...
describe("fileImportsDsModule helper", () => {
  it("matches real @/design-system imports", () => {
    expect(fileImportsDsModule(`import { Button } from "@/design-system/atoms/button";`)).toBe(true);
    expect(fileImportsDsModule(`import { Card } from "@/design-system/composites/card";`)).toBe(true);
  });

  it("excludes the types/meta structural import (not a tier dependency)", () => {
    expect(fileImportsDsModule(`import type { Meta } from "@/design-system/types/meta";`)).toBe(false);
  });
