...
let cwd: string;
beforeEach(async () => { cwd = await freshTmpDir("widen-tokens-"); });
...
function makeCtx(): ProjectContext {
  return {
    ...
    exists: async (p: string) => {
      try { await (await import("node:fs/promises")).stat(join(cwd, p)); return true; } catch { return false; }
    },
    decisions: {},
  };
}
...
describe("widen-tokens migration Op", () => {
  ...
  it("adds all four groups when tokens.json has none of them", async () => {
    ...
    const after = JSON.parse((changes[0] as { kind: "write"; after: Buffer }).after.toString("utf8")) as Record<string, unknown>;
  ...
  it("motion group has duration and ease sub-keys", async () => {
    ...
    const after = JSON.parse((change as { kind: "write"; after: Buffer }).after.toString("utf8")) as Record<string, unknown>;
