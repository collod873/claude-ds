...
let cwd: string;
beforeEach(async () => { cwd = await freshTmpDir("move-tier-file-"); });
...
function fakeCtx(): ProjectContext {
  return {
    cwd,
    cfg: {
      version: "v0.8.0", pack: "next-react", mode: "warn",
...
function findWrite(changes: Change[], path: string): Extract<Change, { kind: "write" }> | undefined {
  return changes.find((c): c is Extract<Change, { kind: "write" }> => c.kind === "write" && c.path === path);
}
...
describe("moveTierFile Op", () => {
  ...
  it("ensureMeta flips an existing kind when it differs from the requested kind", async () => {
    ...
    const op = moveTierFile(
      "design-system/atoms/combo.tsx",
      "design-system/composites/combo.tsx",
      { kind: "composite" },
    );
    const changes = await op.plan(fakeCtx());
    expect(changes).toHaveLength(2);
  ...
  it("apply: rename-only when no ensureMeta — destination has byte-identical source", async () => {
    ...
    const op = moveTierFile(
      "src/components/feat.tsx",
      "features/invoicing/feat.tsx",
    );
    const report = await run(fakeCtx(), [op], "apply");
    expect(report.failed).toBeUndefined();
