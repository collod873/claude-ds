...
describe("fix-pass", () => {
	...
	describe("runFixPass", () => {
		...
		it("DRIFT-MISPLACED is report-only — runFixPass never moves the file (ADR-0015)", async () => {
			...
			const buttonStillInComposites = await stat(
				join(dir, "design-system/composites/button.tsx"),
			).catch(() => null);
			const buttonInAtoms = await stat(join(dir, "design-system/atoms/button.tsx")).catch(
				() => null,
			);
		...
		it("deduplicates Changes for the same path", async () => {
			...
			const writePaths = result.applied.filter((c) => c.kind === "write").map((c) => c.path);
