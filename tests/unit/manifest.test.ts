...
import { isManifestOrKeepfile, ManifestError, parseManifest } from "../../src/lib/manifest";
...
describe("parseManifest", () => {
	...
	it("rejects unknown category", () => {
		expect(() =>
			parseManifest(JSON.stringify({ files: [{ path: "x", category: "weird" }] })),
		).toThrow(ManifestError);
	});
	it("requires format on hybrid entries", () => {
		expect(() =>
			parseManifest(JSON.stringify({ files: [{ path: "x", category: "hybrid" }] })),
		).toThrow(ManifestError);
	});
});
