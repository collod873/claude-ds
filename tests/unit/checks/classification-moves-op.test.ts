...
import { classificationMovesOp, type ClassificationFinding } from "../../../src/lib/checks/classification";
...
describe("classificationMovesOp (sub-issue #228)", () => {
  ...
  it("emits a rename Change for each finding (atoms↔composites)", async () => {
    ...
    const findings: ClassificationFinding[] = [
      { file: join(cwd, "design-system", "atoms", "combobox.tsx"), currentTier: "atom", shouldBe: "composite" },
    ];
    const op = classificationMovesOp(findings);
    const changes = await op.plan(makeCtx());

    const renames = changes.filter((c): c is Extract<Change, { kind: "rename" }> => c.kind === "rename");
  ...
  it("skips node_modules / .git / dist", async () => {
    ...
    await writeFile(join(cwd, "design-system", "atoms", "x.tsx"), `export function X() { return null; }\n`);
    await writeFile(
    ...
    const findings: ClassificationFinding[] = [
      { file: join(cwd, "design-system", "atoms", "x.tsx"), currentTier: "atom", shouldBe: "composite" },
    ];
    const op = classificationMovesOp(findings);
    const changes = await op.plan(makeCtx());

    const writeChanges = changes.filter((c): c is Extract<Change, { kind: "write" }> => c.kind === "write");
  ...
  it("applied via run() moves the file and rewrites the consumer import", async () => {
    ...
    await expect(readFile(join(cwd, "design-system", "composites", "combobox.tsx"), "utf8")).resolves.toContain("Combobox");
    await expect(readFile(join(cwd, "design-system", "atoms", "combobox.tsx"), "utf8")).rejects.toThrow();
