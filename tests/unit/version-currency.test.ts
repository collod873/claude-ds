...
describe("checkVersionCurrency", () => {
  ...
  it("normalizes the leading v on either side", () => {
    expect(
      checkVersionCurrency({ pinned: "0.8.0", installed: "v1.1.0" }).upgradeAvailable,
    ).toBe(true);
    expect(
      checkVersionCurrency({ pinned: "v0.8.0", installed: "1.1.0" }).upgradeAvailable,
    ).toBe(true);
  });

  it("compares by minor and patch, not just major", () => {
    expect(
      checkVersionCurrency({ pinned: "v1.0.0", installed: "v1.1.0" }).upgradeAvailable,
    ).toBe(true);
    expect(
      checkVersionCurrency({ pinned: "v1.1.0", installed: "v1.1.1" }).upgradeAvailable,
    ).toBe(true);
    expect(
      checkVersionCurrency({ pinned: "v1.1.1", installed: "v1.1.0" }).upgradeAvailable,
    ).toBe(false);
  });
});
