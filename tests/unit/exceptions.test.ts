...
describe("exceptions — parseExceptions", () => {
				...
				exceptions: [
					{
						...
						path: "design-system/atoms/foo.tsx",
						issue: "#42",
						reason: "tracked workaround",
					},
				],
			}),
		);
				...
				exceptions: [
					{
						...
						path: "design-system/atoms/bar.tsx",
	...
	it("accepts entry without issue (issue is optional at parse time, warned by lint)", () => {
		const ex = parseExceptions(
			JSON.stringify({
				exceptions: [{ rule: "DRIFT-MISPLACED", path: "design-system/atoms/baz.tsx" }],
			}),
		);
		expect(ex).toHaveLength(1);
		...
		const ex = parseExceptions(
			JSON.stringify({
				exceptions: [
					{
						rule: "OWNED-TOKEN-LINT",
						path: "scripts/check-where-chain.sh",
						permanent: true,
						reason: "detector over-match — DB checker, not DS",
					},
				],
			}),
		);
	...
	it("errors on unknown rule ID with a clear message naming the bad ID", () => {
		try {
			parseExceptions(
				JSON.stringify({
					exceptions: [{ rule: "NOT-A-RULE", path: "design-system/atoms/button.tsx", issue: "#1" }],
				}),
			);
	...
	it("errors on old-style rule ID (TOKEN-001 is not a DriftRuleId)", () => {
		expect(() =>
			parseExceptions(
				JSON.stringify({
					exceptions: [{ rule: "TOKEN-001", path: "design-system/atoms/button.tsx", issue: "#1" }],
				}),
			),
	...
	it("rejects bare array shape", () => {
		expect(() =>
			parseExceptions(JSON.stringify([{ rule: "DRIFT-MISPLACED", path: "a.tsx", issue: "#1" }])),
		).toThrow(ExceptionError);
	});

	it("rejects non-array exceptions value", () => {
		expect(() => parseExceptions(JSON.stringify({ exceptions: "bad" }))).toThrow(ExceptionError);
	});
});
...
describe("lintExceptions", () => {
	...
	it("warns when issue field is missing", async () => {
		const ex = parseExceptions(
			JSON.stringify({
				exceptions: [{ rule: "DRIFT-MISPLACED", path: "design-system/atoms/foo.tsx" }],
			}),
		);
	...
	it("warns when referenced issue is closed", async () => {
		const ex = parseExceptions(
			JSON.stringify({
				exceptions: [{ rule: "DRIFT-MISPLACED", path: "design-system/atoms/foo.tsx", issue: "#7" }],
			}),
		);
	...
	it("accumulates warnings across multiple entries", async () => {
		...
		const checker: IssueChecker = vi
			.fn()
			.mockImplementation(async (ref) => (ref === "#5" ? "open" : "closed"));
		const warnings = await lintExceptions(ex, checker);
	...
	it("includes rule and path in each warning object", async () => {
		const ex = parseExceptions(
			JSON.stringify({
				exceptions: [{ rule: "DRIFT-MISPLACED", path: "design-system/atoms/x.tsx" }],
			}),
		);
		const warnings = await lintExceptions(ex);
		expect(warnings[0].rule).toBe("DRIFT-MISPLACED");
	...
	it("warns when an OWNED-* exception has no issue link and is not permanent (#320)", async () => {
		const ex = parseExceptions(
			JSON.stringify({
				exceptions: [{ rule: "OWNED-TOKEN-LINT", path: "scripts/lint-tokens.ts" }],
			}),
		);
		const warnings = await lintExceptions(ex);
		expect(warnings).toHaveLength(1);
	...
	it("warns when an OWNED-* exception references a closed issue (#320)", async () => {
		const ex = parseExceptions(
			JSON.stringify({
				exceptions: [{ rule: "OWNED-TOKEN-LINT", path: "scripts/lint-tokens.ts", issue: "#7" }],
			}),
		);
		const checker: IssueChecker = vi.fn().mockResolvedValue("closed");
		const warnings = await lintExceptions(ex, checker);
...
describe("exceptions — serializeExceptions", () => {
	...
	it("round-trips through parseExceptions", () => {
		const exceptions = [
			...
			{
				rule: "DRIFT-DS-IMPORTS-FEATURE" as const,
				path: "design-system/atoms/bar.tsx",
				reason: "wontfix",
				permanent: true,
			},
