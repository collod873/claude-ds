...
describe("exceptions", () => {
  ...
  it("accepts wrapped {exceptions:[...]} shape (canonical seed format)", () => {
  ...
  it("accepts empty wrapped shape {exceptions:[]}", () => {
    const ex = parseExceptions(JSON.stringify({ exceptions: [] }));
    expect(ex).toHaveLength(0);
  });

  it("rejects bare array shape (old format)", () => {
    expect(() => parseExceptions(JSON.stringify([
      { rule_id: "r1", file: "a.tsx", reason: "x", expiry: "2026-08-01" },
