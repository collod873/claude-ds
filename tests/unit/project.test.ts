...
import { loadPreAdoptProject, loadProject } from "../../src/lib/project";
...
describe("ProjectContext.auditConfig — PRD #266 Phase B", () => {
	...
	it("ctx (including auditConfig) is frozen on return from both factories", async () => {
		...
		expect(() => {
			(adopted as { auditConfig: unknown }).auditConfig = null;
		}).toThrow();
		...
		try {
			...
			expect(() => {
				(preAdopt as { auditConfig: unknown }).auditConfig = null;
			}).toThrow();
		} finally {
			await cleanup(dir2);
		}
