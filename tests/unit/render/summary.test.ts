...
describe("renderChangeSummary", () => {
		...
		const entries: SummaryEntry[] = [
			{
				...
				change: write("design-system/atoms/button.tsx", "old\n", "new\n"),
			},
	...
	it("surfaces config-flag flips first with key callouts", () => {
		...
		const entries: SummaryEntry[] = [
			{
				opName: "rewriteDsImports",
				change: write(
					"design-system/atoms/button.tsx",
					"import x from '@/design-system/foo';\n",
	...
	it("calls out import-only rewrites with rewrite count", () => {
		...
		expect(renderChangeSummary(entries)).toEqual(["M thing.tsx  (2 imports rewritten)"]);
	});
	...
	it("is pure — calling twice returns equal arrays", () => {
		const entries: SummaryEntry[] = [{ opName: "op", change: write("a.tsx", "x\n", "y\n") }];
