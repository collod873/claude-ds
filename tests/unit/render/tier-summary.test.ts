...
describe("renderChangeTierSummary", () => {
  ...
  it("surfaces config-flag flips first, separately from tier counts", () => {
    const before = JSON.stringify(
      { pack: "next-react", meta_kind_strict: false },
      null,
      2,
    );
    const after = JSON.stringify(
      { pack: "next-react", meta_kind_strict: true },
      null,
      2,
    );
    const entries: SummaryEntry[] = [
  ...
  it("collapses aborts to a count", () => {
    const entries: SummaryEntry[] = [
      ...
      { opName: "syncPackFiles", change: abort("design-system/atoms/bar.tsx", "hand-edited managed file") },
      { opName: "syncPackFiles", change: abort("design-system/atoms/baz.tsx", "hand-edited managed file") },
    ];
    const lines = renderChangeTierSummary(entries);
