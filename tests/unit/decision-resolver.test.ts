...
describe("resolveDecisions — three-kind matrix", () => {
	describe("ambiguity", () => {
		it("TTY + answer supplied → returns supplied answer, no prompt", async () => {
			const prompt = vi.fn();
			const out = await resolveDecisions([AMBIG("a1")], { a1: 1 }, { isTTY: true, prompt });
		...
		it("TTY + answer absent → calls prompt and returns its result", async () => {
			...
			const out = await resolveDecisions([AMBIG("a1")], {}, { isTTY: true, prompt });
		...
		it("non-TTY + answer supplied → returns supplied answer, no prompt", async () => {
			const prompt = vi.fn();
			const out = await resolveDecisions([AMBIG("a1")], { a1: "defer" }, { isTTY: false, prompt });
	...
	describe("commitment-gate", () => {
		it("TTY + answer absent → calls prompt", async () => {
			const prompt = vi.fn().mockResolvedValue(0);
			const out = await resolveDecisions([GATE("g1")], {}, { isTTY: true, prompt });
			expect(prompt).toHaveBeenCalledTimes(1);
		...
		it("TTY + answer supplied → returns supplied (no prompt)", async () => {
			const prompt = vi.fn();
			const out = await resolveDecisions([GATE("g1")], { g1: 0 }, { isTTY: true, prompt });
			expect(prompt).not.toHaveBeenCalled();
			expect(out.answers).toEqual({ g1: 0 });
		});
		...
		it("non-TTY → auto-applies (defaultIndex falls back to 0), no prompt, never pending", async () => {
			const prompt = vi.fn();
			const out = await resolveDecisions([GATE("g1")], {}, { isTTY: false, prompt });
			expect(prompt).not.toHaveBeenCalled();
			expect(out.answers).toEqual({ g1: 0 });
			expect(out.pending).toEqual([]);
		});
	...
	describe("automatable", () => {
		it("TTY → returns default, no prompt, never pending", async () => {
			const prompt = vi.fn();
			const out = await resolveDecisions([AUTO("x1", 0)], {}, { isTTY: true, prompt });
			expect(prompt).not.toHaveBeenCalled();
		...
		it("non-TTY → returns default, no prompt, never pending", async () => {
			const prompt = vi.fn();
			const out = await resolveDecisions([AUTO("x1", 1)], {}, { isTTY: false, prompt });
			expect(prompt).not.toHaveBeenCalled();
		...
		it("supplied answer overrides the default", async () => {
			const out = await resolveDecisions([AUTO("x1", 0)], { x1: 1 }, { isTTY: false });
			expect(out.answers).toEqual({ x1: 1 });
		});
	});
	...
	describe("mixed batches", () => {
		it("non-TTY collect mode mixes resolved automatable/commitment-gate with pending ambiguities", async () => {
			const decisions = [AUTO("auto1", 0), GATE("gate1"), AMBIG("amb1"), AMBIG("amb2")];
			const out = await resolveDecisions(decisions, { amb2: 1 }, { isTTY: false, collect: true });
		...
		it("TTY mixes prompted ambiguities with auto-applied gates and automatables", async () => {
			const prompt = vi.fn().mockResolvedValue(0);
			const decisions = [AUTO("auto1", 0), GATE("gate1"), AMBIG("amb1")];
			const out = await resolveDecisions(decisions, {}, { isTTY: true, prompt });
