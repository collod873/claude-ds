...
describe("scanScaffoldPresence", () => {
	let cwd: string;
	beforeEach(async () => {
		cwd = await freshTmpDir("scaffold-presence-");
	});
	afterEach(async () => {
		await cleanup(cwd);
	});
	...
	it("emits 'present:' lines when verbose=true", async () => {
		...
		const manifest = makeManifest([{ path: "design-system/contracts.md", category: "seeded" }]);
		const r = await scanScaffoldPresence(makeFakeCtx(cwd), {
			manifest,
			appDir: "app",
			claudeMdTarget: "CLAUDE.md",
	...
	it("rewrites the manifest 'app/' prefix through appDir for the existence check", async () => {
		...
		const manifest = makeManifest([{ path: "app/page.tsx", category: "managed" }]);
		const r = await scanScaffoldPresence(makeFakeCtx(cwd), {
			manifest,
			appDir: "src/app",
			claudeMdTarget: "CLAUDE.md",
	...
	it("rewrites the manifest 'CLAUDE.md' path through claudeMdTarget", async () => {
		...
		const manifest = makeManifest([{ path: "CLAUDE.md", category: "managed" }]);
		const r = await scanScaffoldPresence(makeFakeCtx(cwd), {
			manifest,
			appDir: "app",
			claudeMdTarget: ".claude/CLAUDE.md",
			verbose: true,
		});
		expect(r.present).toBe(1);
