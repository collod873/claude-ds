...
describe("scanUnexpectedFiles — strict vs open root", () => {
  let cwd: string;
  beforeEach(async () => { cwd = await freshTmpDir("unexpected-"); });
...
describe("scanUnexpectedFiles — deprecated-match", () => {
  ...
  it("classifies a sibling of a deprecated path as a deprecated-match", async () => {
    ...
    const r = await scanUnexpectedFiles({
      ...
      generatedPatterns: [],
      deprecatedPaths: [
        { path: ".claude/skills/badge-system/SKILL.md", since_version: "v0.3.0", reason: "obsolete" },
      ],
      orphanPaths: NO_ORPHANS,
    });

    expect(r.deprecatedMatches.map(f => f.path)).toContain(".claude/skills/badge-system/README.md");
    expect(r.strictFindings.map(f => f.path)).not.toContain(".claude/skills/badge-system/README.md");
  });
});
...
describe("formatStrictWarnings", () => {
  it("emits a remediation line per strict finding and a summary line", () => {
    const lines = formatStrictWarnings(
      [
        { path: "design-system/types/helpers.ts", root: "design-system/", strict: true, deprecatedMatch: null },
      ],
      [],
    );
    expect(lines.some(l => /WARNING\s+unexpected: design-system\/types\/helpers\.ts/.test(l))).toBe(true);
  ...
  it("uses DS-skill phrasing for findings under .claude/skills/", () => {
    const lines = formatStrictWarnings(
      [
        { path: ".claude/skills/custom-lint/SKILL.md", root: ".claude/skills/", strict: true, deprecatedMatch: null },
      ],
      [],
    );
    expect(lines.some(l => /WARNING\s+unexpected \(DS-related\): \.claude\/skills\/custom-lint\/SKILL\.md/.test(l))).toBe(true);
  });
