...
describe("formatFindings", () => {
  it("groups findings by rule ID and emits a single header per rule", () => {
    const lines = formatFindings([
      { ruleId: "DRIFT-MISPLACED", file: "design-system/composites/a.tsx", message: "should be atom" },
      { ruleId: "DRIFT-MISPLACED", file: "design-system/composites/b.tsx", message: "should be atom" },
      { ruleId: "DRIFT-PATTERN-NO-SLOTS", file: "design-system/patterns/x.tsx", message: "no slots" },
    ]);
  ...
  it("uses ERROR prefix for integrity findings", () => {
    const lines = formatFindings([
      { ruleId: "INTEGRITY-UNPARSEABLE", file: "design-system/atoms/broken.tsx", message: "syntax" },
    ]);
