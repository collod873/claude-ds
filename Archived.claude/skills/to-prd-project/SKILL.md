...
This skill writes a PRD and publishes it as a GitHub issue in the current repo (via `gh issue create`, which targets `origin`). It does **not** apply `agent:implement` — labeling happens _after_ sub-issues have been created.
