...
function parseViolations(stderr) {
    const violations = [];
...
export async function reconformCmd(opts) {
    ...
    try {
        ...
        err(`invalid .claude-ds.json: ${e.message}`);
        process.exit(2);
    }
    ...
    try {
        ...
        err(`invalid pack manifest: ${e.message}`);
        process.exit(2);
    }
    ...
    const companionsCreated = [];
    for (const { dir: tierDir } of TIERS) {
        if (!(await exists(tierDir)))
            continue;
        let entries;
        try {
        ...
        for (const entry of entries) {
            ...
            if (!entry.endsWith(".tsx"))
                continue;
            if (COMPANION_SUFFIXES.some(s => entry.endsWith(s)))
                continue;
            if (SKIP_PATTERNS.some(re => re.test(entry)))
                continue;
            ...
            for (const companion of companions) {
                if (await exists(companion.path))
                    continue;
                if (dryRun) {
                ...
                if (useGenerator && generatorPath) {
                    const result = spawnSync("node", ["--experimental-strip-types", generatorPath, companion.path, companion.path.replace(cwd + "/", "")], { cwd, encoding: "utf8", timeout: 30_000 });
                    if (result.status === 0) {
    ...
    if (contractsLines < 25 || tokensLines < 25) {
        ...
        if (contractsLines < 25)
            lines.push("  design-system/contracts.md looks like a seed stub (< 25 lines)");
        if (tokensLines < 25)
            lines.push("  design-system/tokens.json looks like a seed stub (< 25 lines)");
