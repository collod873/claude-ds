 ...
 */
export async function scanDeprecated(cwd, deprecatedPaths) {
    const findings = [];
 ...
 */
export async function scanClaudeMdCollision(cwd) {
    const findings = [];
...
export async function reconcileCmd(opts) {
    ...
    // ── Report ─────────────────────────────────────────────────────────────────
    const lines = ["", "reconcile: found the following issues:", ""];
    ...
    for (const f of toDelete) {
        ...
        try {
            ...
            info(`warning: could not delete ${f.path}: ${e.message}`);
            skipped++;
        }
    }
    ...
    for (const f of collisionList) {
        ...
        if (ans === "a") {
            try {
                ...
                info(`warning: could not delete CLAUDE.md: ${e.message}`);
                skipped++;
            }
        }
        else if (ans === "b") {
            try {
                ...
                info(`warning: could not delete .claude/CLAUDE.md: ${e.message}`);
                skipped++;
            }
        }
        else {
