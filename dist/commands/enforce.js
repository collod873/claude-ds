...
export async function enforceCmd(opts) {
    const cwd = opts.cwd ?? process.cwd();
    const cfgPath = join(cwd, ".claude-ds.json");
    if (!(await exists(cfgPath))) {
        err(".claude-ds.json absent; run init or adopt first");
        process.exit(2);
    }
    ...
    if (!opts.yes && !(await confirm(`Flip mode warn → block (open exceptions ≤ ${cfg.enforce_threshold})?`))) {
        info("aborted");
        return;
    }
    cfg.mode = "block";
