...
async function readIfExistsLocal(p) {
    try {
...
export function extractChangelogSections(changelog, pinned, installed) {
...
export async function versionCmd(opts) {
    ...
    if (!opts.offline) {
        ...
        if (r.status === 0) {
            ...
            latest = tags[tags.length - 1] ?? "unknown";
        }
    }
