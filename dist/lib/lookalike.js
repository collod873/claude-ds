...
function isLookalike(canonicalBase, candidateBase) {
    ...
    if (canonicalBase.includes(candidateBase) || candidateBase.includes(canonicalBase))
        return true;
    // Stem-based substring containment
    const cStem = stem(canonicalBase);
    const dStem = stem(candidateBase);
    if (cStem.length >= 4 && dStem.includes(cStem))
        return true;
    if (dStem.length >= 4 && cStem.includes(dStem))
        return true;
