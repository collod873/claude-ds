...
// Injection seam: tests pass in-memory fakes; production uses the real
// gh/safeSh/sh. Same shape as the GhRunner seam in publish-sub-issues.ts /
// review-publish.ts, extended to the two shell helpers this module also needs.
export interface ReviewContextDeps {
  readonly gh: (args: string[]) => string;
  readonly safeSh: (cmd: string) => string;
  readonly sh: (cmd: string) => string;
}

export const fetchPullRequestContext = (
  prNumber: string,
  deps: ReviewContextDeps = { gh, safeSh, sh },
): PullRequestContext => {
  const { gh: ghFn, safeSh: safeShFn, sh: shFn } = deps;
  const prView = JSON.parse(
    ghFn(["pr", "view", prNumber, "--json", "title,body,comments"]),
  ) as {
    title: string;
    body?: string | null;
    comments: {
  ...
  const issueTitle = issueNumber
    ? safeShFn(`gh issue view ${issueNumber} --json title --jq .title`).trim()
    : "";
  const linkedIssue = issueNumber
    ? safeShFn(`gh issue view ${issueNumber} --comments`)
    : "(no linked issue found)";

  const reviews = JSON.parse(
    ghFn(["api", `repos/{owner}/{repo}/pulls/${prNumber}/reviews`]),
  ) as {
...
query($owner:String!,$repo:String!,$number:Int!) {
  ...
  const diff = safeShFn("git diff main...HEAD") || shFn("git diff main..HEAD");
