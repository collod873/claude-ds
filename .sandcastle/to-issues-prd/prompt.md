...
1. Fetch the PRD:
   ...
   Read it carefully. The PRD is the spec — do not add scope, do not redesign. If the PRD is ambiguous, make the most reasonable interpretation and proceed; do not stop to ask.
...
3. Optionally explore the codebase to ground the breakdown in the real shape of the files you'll be cutting through.
...
Break the PRD into **tracer-bullet** vertical slices. Each slice is a thin vertical cut through every layer (schema → logic → CLI → tests), NOT a horizontal slice of one layer.
...
Emit a single `<output>` block as the last thing in your response. The script parses it with a strict schema.
