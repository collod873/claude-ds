...
- **Brownfield onboarding flow (PRD #241):** `adopt → classify → audit` takes any
  existing project from zero DS infrastructure to fully scaffolded with no manual
  fixes. `classify` organizes existing `design-system/` files into atoms/composites
  (prompting only on genuine atom-vs-composite ambiguity); `audit --fix` converges
  drift in a single idempotent pass.
- **#256 — tracking moved off the showcase manifest.** Audit file-tracking now
  ...
  This makes `audit --fix` a single-pass fixed point and removes a consumer
  `TS2352` cast error. For consumers upgrading from a pre-v1.0.0 release, the
  `lift-tracking-manifest` migration (v1.0.0 bucket) applies this automatically;
  released v1.0.0 consumers never had the collision (it was introduced after the
  v1.0.0 tag), so they need no migration.
