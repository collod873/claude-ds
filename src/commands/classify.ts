...
import { type CommandResult, commandError, success } from "../lib/command-result.js";
import {
	type AnswerBag,
	type Decision,
	loadAnswersFile,
...
export async function classifyCmd(opts: {
	...
	if (classified.length > 0) {
		...
		if (composites.length > 0) {
			info(
				`composites/ (${composites.length} file${composites.length === 1 ? "" : "s"} → design-system/composites/):`,
			);
		...
		if (unknowns.length > 0) {
			info(
				`skipped/${unknowns.length} file${unknowns.length === 1 ? "" : "s"} (unknown tier — patterns or unresolved):`,
			);
	...
	if (backfillResults.length > 0) {
		for (const r of backfillResults) {
			if (r.kind === "healed") {
				...
				info(
					`classify: EXTRACTION_NEEDED — ${r.atomRel} has unresolvable symbols [${r.unresolvedSymbols?.join(", ")}]`,
				);
			}
		}
	}
