...
import { err, info, printNextStep, setJsonMode } from "../lib/log.js";
...
import { DEFAULT_CLAUDE_MD_TARGET, detectAppDir, detectClaudeMdCandidates } from "../lib/paths.js";
...
async function patchTsconfigForSrcApp(cwd: string): Promise<void> {
	...
	if (unparseable()) {
		info(
			'warning: could not parse tsconfig.json (comments?); skipping @/design-system/* path injection. Add manually: compilerOptions.paths["@/design-system/*"] = ["../design-system/*"]',
		);
		return;
	}
	if (report.failed) {
...
export async function adoptCmd(opts: {
	...
	if (candidates.length === 0) {
		...
		claudeMdTarget =
			candidates.find((c) => c === ".claude/CLAUDE.md") ??
	...
	if (opts.dryRun) {
		info(
			`[dry-run] would adopt pack=${pack}, mode=warn, app_dir=${appDir}, claude_md_target=${claudeMdTarget}`,
		);
	...
	try {
		...
		// This allows a failing script to be detected as non-fatal before the pack's version lands.
		{
			...
			if ((await exists(buildScriptPath)) && !(await exists(manifestPath))) {
				try {
					...
					info(
						`warning: build-manifest failed (exit ${exitCode}), manifest.json not created. Run manually: node --experimental-strip-types scripts/build-manifest.ts`,
					);
				}
			}
		}
		...
		if (overwrites.length > 0) {
			...
			if (managedOvr.length > 0) {
				lines.push(
					`Overwrote ${managedOvr.length} managed file(s) (pack-owned, previous content replaced):`,
				);
			...
			if (hybridOvr.length > 0) {
				...
				lines.push(
					`Updated ${hybridOvr.length} hybrid file(s) (pack-owned regions updated; user-owned content preserved):`,
				);
		...
		// breadcrumb below.
		info(
			`adopted claude-ds (${pack}, mode=warn). Run 'enforce' when ready. Detected package manager: ${pm}. Regenerate the manifest any time with ${runCmd(pm, "ds:build-manifest")}.`,
		);
		info(
			`CI scripts installed. Run: ${runCmd(pm, "ci:hook-contract")} and ${runCmd(pm, "ci:consistency")}`,
		);
		info(
			`A starter GitHub Actions workflow was seeded at .github/workflows/claude-ds-governance.yml (delete if not on GH Actions). See docs/ci-wiring.md for details.`,
		);
		printNextStep("adopt", {});
