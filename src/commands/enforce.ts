...
import { gate, parseExceptions } from "../lib/exceptions.js";
...
export async function enforceCmd(opts: { yes?: boolean; cwd?: string }) {
	...
	try {
		...
		err(c.red(".claude-ds.json absent; run init or adopt first"));
		process.exit(2);
	}
	...
	try {
		...
		info(
			"→ Next: run 'claude-ds audit', then close some exceptions (or raise enforce_threshold in .claude-ds.json)",
		);
		process.exit(2);
	}
	...
	if (ctx.cfg.mode === "block") {
		info(c.dim("enforce: already in block mode — nothing to do"));
	...
	if (
		!opts.yes &&
		!(await confirm(`Flip mode warn → block (open exceptions ≤ ${ctx.cfg.enforce_threshold})?`))
	) {
		err(c.red("aborted"));
		process.exit(130);
	}
	...
	if (report.failed) {
		err(c.red(`enforce failed: ${report.failed.error}`));
		process.exit(2);
	}
