...
export function analyzeResolution(source: string, fileName = "file.tsx"): ResolutionResult {
	...
	const unresolvedValues = [...valueRefs].filter((name) => !bound.has(name) && !GLOBALS.has(name));
	...
	const typeOnlySymbols = new Set([...unresolvedTypes].filter((name) => !valueRefSet.has(name)));
