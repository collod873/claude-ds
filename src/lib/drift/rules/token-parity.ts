...
function summarizeDiff(diff: ParityDiff, cssFile: string): string {
	...
	if (diff.missingInCss.length > 0) {
		...
		parts.push(
			`${diff.missingInCss.length} token${diff.missingInCss.length === 1 ? "" : "s"} missing from ${cssFile}: ${names}`,
		);
	}
	if (diff.valueMismatch.length > 0) {
		const examples = diff.valueMismatch
			.slice(0, 3)
			.map((m) => `--${m.name} (json=${m.jsonValue} css=${m.cssValue})`)
			.join(", ");
		parts.push(
			`${diff.valueMismatch.length} value mismatch${diff.valueMismatch.length === 1 ? "" : "es"}: ${examples}`,
		);
	}
	if (diff.extraInCss.length > 0) {
		const names = diff.extraInCss.map((t) => `--${t.name}`).join(", ");
		parts.push(
			`${diff.extraInCss.length} CSS variable${diff.extraInCss.length === 1 ? "" : "s"} not in tokens.json: ${names}`,
		);
	}
	return parts.join("; ");
}
...
function appendDeclarationsToRoot(
	css: string,
...
function rewriteDeclarationValues(
	css: string,
	mismatches: { name: string; jsonValue: string }[],
): string {
...
async function fix(finding: DriftFinding, ctx: ProjectContext): Promise<FixResult> {
	...
	try {
		...
		return {
			finding,
			fixed: false,
			message: `could not read ${TOKENS_PATH} — run \`claude-ds sync\` to restore`,
			changes: [],
		};
	}
	...
	if (jsonTokens === null) {
		return {
			finding,
			fixed: false,
			message: `${TOKENS_PATH} is not valid JSON — fix manually before re-running`,
			changes: [],
		};
	}
	...
	if (isClean(diff)) {
		return {
			finding,
			fixed: false,
			message: `${TOKENS_PATH} already in parity with ${cssFile}`,
			changes: [],
		};
	}
	...
	if (diff.missingInCss.length > 0) {
		summary.push(
			`added ${diff.missingInCss.length} missing CSS variable${diff.missingInCss.length === 1 ? "" : "s"}`,
		);
	}
	if (diff.valueMismatch.length > 0) {
		summary.push(
			`updated ${diff.valueMismatch.length} mismatched value${diff.valueMismatch.length === 1 ? "" : "s"} to match ${TOKENS_PATH}`,
		);
	}
...
export const tokenParityRule: DriftRule = {
	...
	severity: "error",
	description:
		"tokens.json and the consumer's emitted CSS variables disagree on tokens — JSON is the source of truth",
	detect,
	fixable: true,
	fix,
	priority: 4,
