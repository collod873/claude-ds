...
export async function applyAmbiguityPass(params: {
		...
		ambiguousAtoms.push({
			...
			decision: {
				...
				options: [
					...
					{
						label: "Move to composites",
						description: "It combines other components and belongs in composites/",
					},
				],
			},
		});
		...
		try {
			...
			const result = await resolveDecisions(
				...
				suppliedAnswers,
				{
					...
					collect,
					prompt: ambiguityPrompt ? async (q, o) => ambiguityPrompt(q, o) : undefined,
				},
			);
	...
	if (exceptionsToAdd.length > 0) {
		...
		info(
			`classify: ${exceptionsToAdd.length} ambiguity exception(s) written to design-system/exceptions.json`,
		);
	}
