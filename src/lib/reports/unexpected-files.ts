...
import { type DeprecatedPath, isManifestOrKeepfile, type ManagedRoot } from "../manifest.js";
...
const DS_KEYWORDS_RE =
	/\b(design[- _]?system|atoms?|composites?|tokens?|design[- _]?tokens|tailwind|css[- _]?variables)\b/i;
...
export async function findUnexpectedFiles(
	...
	const isGenerated =
		generatedPatterns.length > 0 ? picomatch(generatedPatterns, { dot: true }) : null;

	const isIgnored = ignoreGlobs.length > 0 ? picomatch(ignoreGlobs, { dot: true }) : null;
...
export function formatStrictWarnings(
	...
	for (const f of strictFindings) {
		...
		if (isSkill) {
			lines.push(
				`WARNING  unexpected (DS-related): ${f.path} (in ${f.root}) — add to lookalike_ignore in .claude-ds.json, or delete if unintended`,
			);
		} else {
			lines.push(
				`WARNING  unexpected: ${f.path} (in ${f.root}) — add to lookalike_ignore in .claude-ds.json, or delete if unintended`,
			);
		}
	}
	if (strictFindings.length > 0) {
		lines.push(
			`${strictFindings.length} unexpected file(s) under strict managed roots — add to lookalike_ignore in .claude-ds.json to suppress`,
		);
	}
	if (nonDsUnexpected.length > 0) {
		const names = nonDsUnexpected
			...
			.join(", ");
		lines.push(
			`${nonDsUnexpected.length} non-DS skill(s) detected under .claude/skills/ (ignored: ${names})`,
		);
	}
	return lines;
}
