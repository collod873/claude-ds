import type { Finding } from "../lookalike.js";
import type { PackageManager } from "../package-manager.js";
import type { RootDupeFinding } from "../root-dupes.js";
...
function summarizePresentByTier(present: Finding[]): string[] {
	...
	const ordered = [
		...declared.filter((l) => counts.has(l)),
		...order.filter((l) => !declared.includes(l)),
	];
...
export function renderMarkdown(result: DoctorResult, verbose: boolean): string {
	...
	if (result.mode === "pre-adopt") {
		...
		if (lookalikes.length > 0) {
			...
			lines.push(
				"The following files/dirs look like canonical names but use different vocabulary.",
			);
	...
	if (result.rootDupes && result.rootDupes.length > 0) {
		...
		lines.push(
			"These files existed before `adopt` and were not removed. Run `reconcile` to resolve.\n",
		);
		for (const d of result.rootDupes) {
			const note = d.contentDiffers
				? "(content differs — merge required)"
				: "(content identical — safe to delete root)";
