	...
	for (const hookFile of hookFiles.sort()) {
		...
		if (!(await exists(hookPath))) {
			results.push({
				hook: hookFile,
				status: "FAIL",
				reason: "hook script missing from adopted project",
			});
			continue;
		}
			...
			if (code === 0) {
				...
				if (STDERR_CONTRACT.test(stderr)) {
					results.push({
						hook: hookFile,
						status: "FAIL",
						reason: "hook blocked pass payload (exit 2)",
					});
				} else {
					results.push({
						hook: hookFile,
						status: "FAIL",
						reason: "stderr does not match contract",
					});
				}
			} else if (code === 1) {
				...
				results.push({
					hook: hookFile,
					status: "FAIL",
					reason: `hook self-error (exit 1): ${stderr.slice(0, 120).trim()}`,
				});
			} else {
