...
//
// Both the CLI's showcase generator (`src/lib/showcase/generator.ts`, #567) and
// every CVA-consuming fixer import this single implementation — there is no
// second parser to drift (PRD #546). The pack used to inline a byte-for-byte
// copy here (its generator could not import the CLI's src tree); that copy and
// its sync machinery are gone now that the pack ships a thin shim delegating to
// `claude-ds regen-showcases` (issue #568, ADR-0031).
//
// The TypeScript compiler module is injected (never imported here) so callers
// supply their own `typescript`. Analysis is purely syntactic (a single
// `createSourceFile`, no type checker), which keeps it cheap and portable.
