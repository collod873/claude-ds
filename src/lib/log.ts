...
export function setJsonMode(active: boolean): void {
...
export function printNextStep(command: NextStepCommand, ctx: NextStepContext): void {
	...
	switch (command) {
		case "adopt":
			...
			message =
				"have loose components to organize? run 'claude-ds classify --src <their-dir>' to migrate them";
			verify = true;
			break;
		case "classify":
		...
		case "audit":
			...
			if ((ctx.extractionCount ?? 0) > 0) {
				...
				message =
					"run 'claude-ds heal' to auto-repair, or 'claude-ds audit --except' to register exceptions";
		...
		case "sync":
			...
			if (ctx.brownfield) {
				message =
					"run 'claude-ds heal --allow-dirty' to organize the existing design-system files sync just wrote";
			} else {
...
export async function confirm(question: string): Promise<boolean> {
	if (!process.stdin.isTTY) {
		err(
			`${question} [y/N]: non-TTY, no answer available — pass --yes to confirm non-interactively`,
		);
		process.exit(3);
	}
