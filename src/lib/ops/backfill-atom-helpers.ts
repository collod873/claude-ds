 ...
 * and imports.
 */
function parseLocalsAndImports(source: string, fileName: string): {
  locals: LocalDecl[];
  imports: ImportInfo[];
} {
  ...
  for (const stmt of sf.statements) {
    ...
    if (ts.isFunctionDeclaration(stmt) && stmt.name) {
      ...
      names = [stmt.name.text]; typeOnly = true;
...
function computeHelperClosure(
  ...
  const sf = ts.createSourceFile(
    compositeFileName, compositeSource, ts.ScriptTarget.Latest, true, ts.ScriptKind.TSX,
  );
...
function insertHelpersAfterImports(source: string, helpers: string): string {
  const sf = ts.createSourceFile("_tmp.tsx", source, ts.ScriptTarget.Latest, true, ts.ScriptKind.TSX);
    ...
    async plan(ctx: ProjectContext): Promise<Change[]> {
      ...
      for (const atomRel of atomFiles) {
        ...
        if (parentLocalExported.size > 0) {
          if (!atomSource.includes(BACKFILL_EXTRACTION_NEEDED_MARKER)) {
            const marked = BACKFILL_EXTRACTION_NEEDED_MARKER +
        ...
        if (ownerComps.size > 1) {
          ...
          if (!atomSource.includes(BACKFILL_EXTRACTION_NEEDED_MARKER)) {
            const marked = BACKFILL_EXTRACTION_NEEDED_MARKER +
              " — helpers span multiple composites: " + [...parentLocalPrivate].join(", ") + "\n" + atomSource;
            changes.push({
              kind: "write",
              path: atomRel,
              before: Buffer.from(atomSource),
        ...
        const closure = computeHelperClosure(parentLocalPrivate, ownerComp.locals, ownerComp.source, ownerComp.rel);
        if (closure === null) {
          // Closure hit an exported decl or a missing symbol — cannot safely carry.
          if (!atomSource.includes(BACKFILL_EXTRACTION_NEEDED_MARKER)) {
            const marked = BACKFILL_EXTRACTION_NEEDED_MARKER +
              " — closure contains exported or missing symbols: " + [...parentLocalPrivate].join(", ") + "\n" + atomSource;
            changes.push({
              kind: "write",
              path: atomRel,
              before: Buffer.from(atomSource),
        ...
        const atomSf = ts.createSourceFile(atomRel, atomSource, ts.ScriptTarget.Latest, true, ts.ScriptKind.TSX);
        ...
        const stillMissingParentLocal = [...parentLocalPrivate].filter(nm => afterUnresolved.includes(nm));
        if (stillMissingParentLocal.length > 0) {
          // Still unresolved after repair attempt → honesty fallback
          if (!atomSource.includes(BACKFILL_EXTRACTION_NEEDED_MARKER)) {
            const marked = BACKFILL_EXTRACTION_NEEDED_MARKER +
              " — unresolved after repair attempt: " + stillMissingParentLocal.join(", ") + "\n" + atomSource;
            changes.push({
              kind: "write",
              path: atomRel,
              before: Buffer.from(atomSource),
