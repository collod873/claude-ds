...
describe("generate-showcase-companion.ts [integration] — ATOM fixture cluster", () => {
  ...
  beforeAll(async () => {
    ...
    statesParsed = JSON.parse(await readFile(join(dsDir, "button.states.json"), "utf8"));
  });
  ...
  it("emits .states.json with __generated marker for atom", () => {
    expect(statesParsed).toHaveProperty("__generated");
    expect(typeof statesParsed.__generated).toBe("string");
    expect(statesParsed.__generated).toMatch(/@generated by claude-ds/);
    expect(Array.isArray(statesParsed.states)).toBe(true);
    expect(statesParsed.states.length).toBeGreaterThan(0);
  });
...
describe("generate-showcase-companion.ts [integration] — CVA fixture cluster", () => {
  ...
  beforeAll(async () => {
    ...
    statesParsed = JSON.parse(await readFile(join(dsDir, "badge.states.json"), "utf8"));
  });
  ...
  it("states.json for CVA component contains correct number of states (cross-product minus skip)", () => {
    // 1 explicit default + (3 sizes × 2 tones = 6 combos minus 1 skip = 5) CVA = 6 total
    expect(statesParsed.states).toHaveLength(6);
  });
...
describe("v0.7.0 hand-off contract (issue #60) — no-analyzer cluster", () => {
  ...
  it("(b) full CVA variant matrix rendered (3 intents × 3 sizes = 9 combos)", () => {
    ...
    expect(statesParsed.states.length).toBeGreaterThanOrEqual(9);
  });

  it("(c) meta.states.loading produces a Loading row in output", () => {
    expect(showcaseContent).toContain(">States<");
    expect(showcaseContent).toContain(">Loading<");
    expect(showcaseContent).toContain("loading-default");
    expect(showcaseContent).toContain("loading");
    expect(showcaseContent).toContain(">Long text<");
    expect(showcaseContent).toContain("long-text");
  });
...
describe("v0.7.8 showcase format finalization (issue #65) — no-analyzer cluster", () => {
  ...
  it("(#4) States section renders a row per declared state with force mechanism", () => {
  ...
  it("(#91) containment wrapper does NOT appear on in-flow state rows (hover/focus/disabled/pressed/expanded/invalid)", () => {
    const containmentClass = "relative overflow-hidden rounded border border-dashed border-border max-h-64 min-h-32 grid place-items-center";
...
describe("v0.7.8 showcase format finalization (issue #65) — bespoke", () => {
  let dir: string;
...
describe("AST-based meta extractor (A3) — data-grid fixture", () => {
  ...
  beforeAll(async () => {
    ...
    statesParsed = JSON.parse(await readFile(join(dsDir, "data-grid.states.json"), "utf8"));
  });
  ...
  it("emits states.json with empty state props from states block", () => {
    expect(statesParsed).toHaveProperty("__generated");
    expect(Array.isArray(statesParsed.states)).toBe(true);
